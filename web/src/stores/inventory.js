...
function canStackItems(a, b) {
	if (!a || !b) return false;
	if (a.name !== b.name || a.type === "item_weapon" || b.type === "item_weapon")
		return false;
	...
	if (aMax > 0) {
		return (
			...
			Number(a.degradation) === Number(b.degradation)
		);
	}
	return true;
}
...
export const useInventoryStore = defineStore("inventory", () => {
	...
	const weaponAmmoHud = ref({ show: false });
		...
		switch (data.action) {
			...
			case "display":
				...
				if (data.type === "custom") {
					...
					secondWeight.value = data.weight || null;
				}
				...
				if (data.type === "store") {
					...
					geninfo.value = data.geninfo;
				}
				if (data.type === "steal") {
					stealid.value = data.stealId;
					secondWeight.value = data.weight || null;
				}
			...
			case "setItems":
				...
				if (LuaConfig.value.AddAmmoItem) {
					items.push({
						name: "gunbelt",
						label: LANGUAGE.value.gunbeltlabel || "Gun Belt",
						desc: LANGUAGE.value.gunbeltdescription || "Your Ammo",
						count: 1,
						type: "item_ammo",
						weight: 0,
						group: 1,
				...
				if (LuaConfig.value.AddDollarItem) {
					items.push({
						name: "money",
						label: LANGUAGE.value.inventorymoneylabel || "Money",
						count: money.value || 0,
						type: "item_money",
						weight: 0,
						group: 1,
				...
				if (LuaConfig.value.AddGoldItem) {
					items.push({
						name: "gold",
						label: LANGUAGE.value.inventorygoldlabel || "Gold",
						count: gold.value || 0,
						type: "item_gold",
						weight: 0,
						group: 1,
			...
			case "weaponAmmoHud":
				weaponAmmoHud.value = data.info || { show: false };
				break;
	...
	function addNotification(type, name, label, count) {
		...
		setTimeout(function () {
			...
			for (var i = 0; i < itemNotifications.value.length; i++) {
				if (itemNotifications.value[i].id === id) {
					idx = i;
					break;
				}
			}
	...
	function swapSlots(fromSlot, toSlot, amount) {
		...
		for (var i = 0; i < playerInventory.value.length; i++) {
			if (playerInventory.value[i].slot === fromSlot) {
				fromItem = playerInventory.value[i];
				fromIdx = i;
			}
			if (playerInventory.value[i].slot === toSlot) {
				toItem = playerInventory.value[i];
				toIdx = i;
			}
		}
		...
		if (fromItem.type === "item_weapon") {
			if (toItem && toIdx !== -1) {
				playerInventory.value[toIdx] = Object.assign({}, toItem, {
					slot: fromSlot,
				});
				postNUI("UpdateSlot", {
					itemId: toItem.id,
					slot: fromSlot,
					itemType: toItem.type,
				});
			}
			playerInventory.value[fromIdx] = Object.assign({}, fromItem, {
				slot: toSlot,
			});
			postNUI("UpdateSlot", {
				itemId: fromItem.id,
				slot: toSlot,
				itemType: fromItem.type,
			});
			return;
		}
		...
		if (!toItem) {
			if (moveAmount < fromItem.count) {
				...
				playerInventory.value.push(
					Object.assign({}, fromItem, { count: moveAmount, slot: toSlot }),
				);
				postNUI("SplitSlot", {
					itemId: fromItem.id,
					fromSlot: fromSlot,
