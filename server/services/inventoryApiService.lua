...
function InventoryAPI.addItem(source, name, amount, metadata, cb, allow, degradation, percentage)
	...
	DBService.CreateItem(charIdentifier, svItem:getId(), amount, metadata or {}, name, isExpired, function(craftedItem)
		...
		-- Auto-assign slot to new item
		local usedSlots = {}
		for _, existingItem in pairs(userInventory) do
		...
		local freeSlot = 1
		while usedSlots[freeSlot] do freeSlot = freeSlot + 1 end
		newItem:setSlot(freeSlot)
		DBService.UpdateItemSlot(charIdentifier, craftedItem.id, freeSlot)
...
function InventoryAPI.registerWeapon(_target, wepname, ammos, components, comps, cb, wepId, customSerial, customLabel, customDesc)
	...
	DBService.insertAsync(query, params, function(result)
		...
		-- Auto-assign slot
		local usedSlots = {}
		local userInventory = UsersInventories.default[targetIdentifier] or {}
		for _, existingItem in pairs(userInventory) do
			if existingItem:getSlot() then usedSlots[existingItem:getSlot()] = true end
		end
		for _, wp in pairs(UsersWeapons.default) do
			if wp.propietary == targetIdentifier and wp:getSlot() then usedSlots[wp:getSlot()] = true end
		end
		local freeSlot = 1
		while usedSlots[freeSlot] do freeSlot = freeSlot + 1 end
		...
		DBService.UpdateWeaponSlot(weaponId, freeSlot)
		TriggerEvent("syn_weapons:registerWeapon", weaponId)
		TriggerClientEvent("vorpInventory:receiveWeapon", _target, weaponId, targetIdentifier, name, ammo, label, serialNumber, label, _target, desc, weight, freeSlot, 0, 100)
		TriggerClientEvent("vorpInventory:itemNotify", _target, "add", name, label, 1)
	end)
	return respond(cb, true)
end
...
function InventoryAPI.giveWeapon(player, weaponId, target, cb)
	...
	if weapon then
		...
		DBService.updateAsync(query, params, function(r)
			...
			TriggerClientEvent("vorpInventory:receiveWeapon", _source, weaponId, weaponPropietary, weaponName, weaponAmmo, label, serialNumber, customLabel, _source, customDesc, weight, nil, 0, 100)
		end)
	end
	return respond(cb, true)
end
