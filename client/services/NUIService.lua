...
function NUIService.NUIDropItem(obj)
	...
	if isvalid then
		...
		print("[vorp_inventory] NUIDropItem: type=" .. tostring(type) .. " qty=" .. tostring(qty) .. " item=" .. tostring(itemName))
...
function NUIService.NUIUseItem(data)
	...
	elseif data.type == "item_weapon" then
		...
		if weaponId and UserWeapons[weaponId] then
			...
			SendNUIMessage({ action = "itemNotification", ["type"] = "used", name = weapon:getName(), label = weapon:getLabel(), count = 1 })
		end
	end
end
...
function NUIService.NUIPickupFromDrop(data, cb)
	if not data then if cb then cb('ok') end return end
	TriggerServerEvent("vorpinventory:pickupFromDrop", data.dropId, data.fromSlot, data.targetSlot, data.amount, data.name, data.type, data.isMoney, data.isGold, data.uuid, data.uids)
...
RegisterNetEvent("vorpInventory:setCraftItems", function(items, recipeData)
	SendNUIMessage({ action = "setCraftItems", items = items, recipe = recipeData })
end)

RegisterNetEvent("vorpInventory:craftStarted", function(timer)
	SendNUIMessage({ action = "craftStarted", timer = timer })
end)

RegisterNetEvent("vorpInventory:craftCompleted", function(rewardName)
	SendNUIMessage({ action = "craftCompleted", rewardName = rewardName })
end)

RegisterNetEvent("vorpInventory:itemNotify", function(nType, name, label, count)
	SendNUIMessage({ action = "itemNotification", ["type"] = nType, name = name, label = label, count = count })
end)

RegisterNetEvent("vorpInventory:usedNotification", function(name, label)
	SendNUIMessage({ action = "itemNotification", ["type"] = "used", name = name, label = label, count = 1 })
end)
