...
InventoryApiService.addItem = function(itemData)
    ...
    local prevCount = item and item:getCount() or 0
    local added = itemAmount - prevCount

    if item then
        item:setCount(itemAmount)
    else
        if not itemData.slot and findNextFreeSlot then
            itemData.slot = findNextFreeSlot()
        end
