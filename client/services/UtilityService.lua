...
function Utils.useWeapon(id)
    ...
    if not id or not UserWeapons[id] then
        return
    end
    ...
    if UserWeapons[id]:getUsed2() then
        GiveDelayedWeaponToPed(playerPedId, weaponHash, 0, true, 0)
        ...
        SetCurrentPedWeapon(playerPedId, weaponHash, true, 3, false, false)

        Wait(150)

        local _, currentWeapon = GetCurrentPedWeapon(playerPedId, false, 0, false)
        if currentWeapon ~= weaponHash then
            SetCurrentPedWeapon(playerPedId, weaponHash, true, 0, false, false)
        end

        SetPedAmmo(playerPedId, weaponHash, 0)

        for ammoType, ammoAmount in pairs(UserWeapons[id]:getAllAmmo()) do
            SetPedAmmoByType(playerPedId, joaat(ammoType), ammoAmount)
        end
    else
